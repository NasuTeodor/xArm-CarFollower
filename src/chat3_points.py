...
while True:
    ...
    for cnt in contours:
        ...
        x0_px, y0_px = cnt[0][0]
        x0_mm, y0_mm = px_to_robot((x0_px, y0_px))
        arm.set_position(x=x0_mm, y=y0_mm, z=Z_SAFE, wait=True, speed=SPEED_TRANSIT)
        arm.set_position(x=x0_mm, y=y0_mm, z=Z_DRAW, wait=True, speed=SPEED_DRAW)

        # trace points
        for p in cnt:
            x_px, y_px = p[0]
            x_mm, y_mm = px_to_robot((x_px, y_px))
            arm.set_position(x=x_mm, y=y_mm, z=Z_DRAW, wait=True, speed=SPEED_DRAW)

        # lift after each contour
        arm.set_position(z=Z_SAFE, relative=True, wait=True, speed=SPEED_TRANSIT)
        key = cv2.waitKey(1) & 0xFF
